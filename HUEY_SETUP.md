...
| `HUEY_THREADS` | `2` | integer | Number of worker threads |
...
# Or run directly (module form avoids PATH/console-script issues)
python -m huey.bin.huey_consumer poi_broker.worker.huey --workers=2 --worker-type=thread --verbose
```

> **Important:** always point the consumer at **`poi_broker.worker.huey`**, never
> `poi_broker.extensions.huey`. `worker.py` imports `poi_broker.tasks`, which
> registers `create_export_file` on the shared Huey instance. A worker started
> from `extensions.huey` runs but never executes any task.
...
The worker is a long-running process and **must be supervised** so a crash cannot
leave exports permanently stuck. Two templates are provided in `deploy/`:

- `deploy/poi-broker-huey.service` — **systemd** unit (recommended for Linux).
- `deploy/supervisord-poi-broker-huey.conf` — portable **supervisord** config.

Both restart the worker automatically. Without supervision, a dead worker leaves
`PENDING`/`RUNNING` exports stuck; the stale-task guard (below) recovers the DB
state, but the exports still need a live worker to run.

### Stale-task guard

Exports stuck in `PENDING`/`RUNNING` for **30 minutes** are automatically marked
`FAILED`:
- **At app startup** (web + worker) so a crash/restart doesn't permanently block
  a user's active-task slot.
- **Every 5 minutes** via the periodic task `cleanup_stale_export_tasks`.

This frees the "active export" slot so the user can retry. Adjust the threshold
with `poi_broker/tasks.py::STALE_TASK_MAX_AGE` (seconds).
...
4. **Verify the consumer targets `poi_broker.worker.huey`** (not `extensions.huey`). A worker started against `extensions.huey` runs but never executes tasks.
5. Verify `HUEY_SQLITE_PATH` is set to the **same absolute path** in both the app and the worker (a mismatched path creates two separate empty queues).
6. Restart both app and worker with the same environment variables
...
4. Run with verbose logging: `python -m huey.bin.huey_consumer poi_broker.worker.huey --verbose --logfile=-` (output to console)
...
For better concurrency, run multiple workers. **Each must target
`poi_broker.worker.huey`** so tasks are registered on the shared Huey instance:
...
> For a single-server deployment, one worker with 4 threads is usually
> sufficient and avoids SQLite lock contention. See
> `deploy/PRODUCTION_DEPLOYMENT.md` for the full Ubuntu/Nginx/Gunicorn setup.

### Process Manager (systemd, supervisord)

A complete, ready-to-use systemd unit is provided at
`deploy/poi-broker-huey.service` (and a supervisord alternative at
`deploy/supervisord-poi-broker-huey.conf`). Example systemd service:
...
ExecStart=/opt/poi_broker/.venv/bin/python -m huey.bin.huey_consumer poi_broker.worker.huey --workers=2 --worker-type=thread --logfile=/var/log/poi_broker/huey.log
Restart=always
RestartSec=10
