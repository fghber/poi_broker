...
REM NOTE: target poi_broker.worker.huey (imports tasks so they register on the
REM shared Huey instance). Pointing at poi_broker.extensions.huey would start a
REM worker that never executes any task.
REM The importable module is huey.bin.huey_consumer (NOT huey_consumer, which is
REM only a console-script entry point). Using `python -m` avoids PATH issues.
python -m huey.bin.huey_consumer ^
